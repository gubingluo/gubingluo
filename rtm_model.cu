...
// commom parameter
	...
	if (flag_layout == 1)
	{
		...
		for (ishot = startshot; ishot <= nshot; ishot=ishot+dshot)
		{
			...
			headerkey.iTrace = 1;
			headerkey.Offset = (int)(nx1*dx - spx);        // 起始道偏移距，其余道按dx逐个增加
			headerkey.sx = (int)(spx);
			headerkey.gx = (int)(nx1*dx);                  // 起始道位置，其余道按dx逐个增加
	...
	else  if (flag_layout == 3)
	{
		...
		for (ishot = startshot; ishot <= nshot; ishot=ishot+dshot)
		{
			...
			headerkey.iTrace = 1;
			headerkey.Offset = (int)(offset);                       // 起始道偏移距，其余道按dx逐个增加
			headerkey.sx = (int)(spx);
			headerkey.gx = (int)(offset + nx1*dx);                  // 起始道位置，其余道按dx逐个增加
			headerkey.Nt = (unsigned short)(nt);
			headerkey.Dt = (unsigned short)(dt*1000000);
			headerkey.Dx = dx;
